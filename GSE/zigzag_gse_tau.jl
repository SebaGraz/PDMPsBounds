...
function next_ht(x, v, f, b, mp, N, prparam)
    # return Inf, 0, 1, ()
    d = length(v)
    nt = Inf
    j = 0
    ...
    for i in 1:d
        htime = ht(i, x, v, f, b)
        rtime, rpbs = rand_time_tau(i, x, f, b, v, mp, N, prparam)
...
function rand_time_tau(k, t, f, b, v, mp, N, prparam)
    λ1, ν1, λ2, ν2 =  prparam 
    m = length(v)
    c = 0.0
    aa = λ1
    ...
    τ2 = poisson_time_inv2(cc, dd, rand())
    @test τ1 > 0.0
    @test τ2 > 0.0
    τ = min(τ1, τ2) 
    return τ, (aa, bb, cc, dd)
end

function λtau_up(δt, aa, bb, cc, dd)
    return max(1/(aa + bb*δt), 0) + max(1/(cc + dd*δt), 0.0)
end

function λtau(δt, aa, bb, cc, dd)
    max(1/(aa + bb*δt) + 1/(cc + dd*δt), 0.0)
end
...
function run_zz(t_, v_, f_, b_, T, mp, N::Int64, prparam::Tuple{Float64, Float64, Float64, Float64}; verbose = false)
    tote = 0 
    re = 0
    ...
    println("marignalizing over β and γ")
    Ξ = [(t0, t_),]
    perc = T /10
    while true
        δt, j, tag, pbs1 = next_ht(t, v, f, b, mp, N, prparam)
        δt > 0.0 || error(" δht = $(δht)")
        t[1:d] = t[1:d] + v*δt
        t0 += δt
        if t0 > perc
...
function runall()
    ...
    Ξ, t, v, f, b = run_zz(t0, v, f0, b0, T, mp, N,  par_prior; verbose = false);
